...
async function main() {
  ...
  // Use defaults for display if not available
  const displayAvg = avgProcessingDays || 127;
  const displayMedian = medianProcessingDays || 89;
  ...
  const reportTitle = "Portland Progress Report \u2014 Q1 2026";
  const reportSummary = "The inaugural Portland Progress Report combines dashboard data with narrative analysis to tell the story behind Portland's numbers. This quarter: a deep dive into the permitting crisis, the business formation landscape, and what the data means for Portland's future.";

  const [report] = await sql`
    INSERT INTO content.progress_reports (title, issue_date, slug, summary, published)
    VALUES (${reportTitle}, '2026-03-15', ${slug}, ${reportSummary}, true)
    RETURNING id
  `;

  const reportId = report.id;

  // Build content strings as variables (avoid template literal confusion in tagged SQL)
  ...
  const section1Snapshot = sql.json({
    permits: totalPermits,
    businesses: totalBusinesses,
    crimesPerMonth,
    avgProcessingDays: displayAvg,
    medianProcessingDays: displayMedian,
    permitBacklog,
  });

  await sql`
    INSERT INTO content.report_sections (report_id, title, subtitle, content, section_order, section_type, data_snapshot)
    VALUES (${reportId}, ${"By the Numbers"}, ${"Portland's vital signs, measured."}, ${section1Content}, ${1}, ${"data-summary"}, ${section1Snapshot})
  `;
  ...
  const section2Content = [
    ...
    "## The numbers",
    "",
    "Our dashboard tracks **" + totalPermits.toLocaleString() + " permits** in the Portland system. Of those with processing time data, the average permit takes **" + displayAvg + " days** from application to issuance. The median sits at **" + displayMedian + " days** \u2014 meaning half of all permits take nearly three months just to get approved.",
    "",
  ...
  const section2Snapshot = sql.json({
    permits: totalPermits,
    avgProcessingDays: displayAvg,
    medianProcessingDays: displayMedian,
    permitBacklog: permitBacklog || 0,
    peerCities: [
      { city: "Portland", avgDays: displayAvg },
  ...
  await sql`
    INSERT INTO content.report_sections (report_id, title, subtitle, content, section_order, section_type, data_snapshot)
    VALUES (${reportId}, ${"The Permitting Crisis: What the Data Shows"}, ${"Portland's building permit process is the hidden bottleneck holding back the city's recovery."}, ${section2Content}, ${2}, ${"article"}, ${section2Snapshot})
  `;
  ...
  await sql`
    ...
    VALUES (${reportId}, ${"Business Spotlight"}, ${"Portland's entrepreneurial ecosystem in focus."}, ${section3Content}, ${3}, ${"profile"})
  `;
  ...
  await sql`
    INSERT INTO content.report_sections (report_id, title, subtitle, content, section_order, section_type)
    VALUES (${reportId}, ${"Policy Watch"}, ${"Data-informed recommendations for Portland's leaders."}, ${section4Content}, ${4}, ${"recommendation"})
  `;
