    ...
    <div className="space-y-4">
      <div className="rounded-sm border border-[var(--color-parchment)] bg-white">
        <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 border-b border-[var(--color-parchment)] px-5 pt-4 pb-3 sm:px-6">
          ...
          <p className="font-mono text-[11px] text-[var(--color-ink-muted)]">width = share of shelter users (Kuhn &amp; Culhane); people and cost per year = 2026 proposal&apos;s assumptions</p>
        </div>
        <div className="px-5 py-4 sm:px-6">
            ...
            {LANES.map((l, i) => (
              <div key={l.id} className="flex flex-col justify-between rounded-sm px-3 py-2" style={{ width: `${SHARE[i]}%`, backgroundColor: `color-mix(in srgb, ${l.color} 18%, white)`, borderTop: `4px solid ${l.color}` }}>
                ...
                <div className="flex flex-wrap items-baseline gap-x-3 gap-y-0 font-mono text-[11px] tabular-nums text-[var(--color-ink)]">
                  ...
                  {SHARE[i] >= 20 ? <span className="hidden sm:inline">{PEOPLE[i]} people a year · {COST[i]} each (proposal)</span> : null}
                </div>
              </div>
            ))}
          </div>
          <div className="mt-3 grid gap-2 sm:grid-cols-3">
            {LANES.map((l, i) => (
              <p key={l.id} className="font-mono text-[10.5px] leading-snug text-[var(--color-ink-light)]"><span className="font-semibold" style={{ color: l.color }}>{l.name.split(" · ")[0]}</span> · {PEOPLE[i]} people a year at {COST[i]} each, by the 2026 proposal&apos;s assumptions</p>
            ))}
          </div>
