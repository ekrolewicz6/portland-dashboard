 ...
 *
 * Usage: npx tsx ingest/fetch-hud-vacancy.ts
 *        HUD_API_TOKEN=xxx npx tsx ingest/fetch-hud-vacancy.ts
 */
...
async function tryHudApi(): Promise<HudVacancyRow[] | null> {
  ...
  if (!token) {
    ...
    console.log("  Then run: HUD_API_TOKEN=your_token npx tsx ingest/fetch-hud-vacancy.ts");
    return null;
  }
...
async function main() {
  ...
  try {
    ...
    if (rows && rows.length > 0) {
      ...
      console.log("  HUD_API_TOKEN=your_token npx tsx ingest/fetch-hud-vacancy.ts");
    }
